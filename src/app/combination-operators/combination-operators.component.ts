...
export class CombinationOperatorsComponent {
  ...
  demoCombineLatest() {
    this.clear();
    this.currentCode = `// combineLatest(): 任一 observable 發出值時組合所有最新值\ncombineLatest([a$, b$]) => [最新a, 最新b]\n
const a$ = interval(1000).pipe(take(3));
const b$ = interval(1500).pipe(take(2));
...
fast$.pipe(withLatestFrom(slow$)).subscribe(([f, s]) => console.log(f, s));`;
...
forkJoin([a$, b$]).subscribe(([a, b]) => console.log(a, b));`;
  ...
  demoZip() {
    this.clear();
    this.currentCode = `// zip(): 將 observable 依順序組合發出
zip([a$, b$]) => [a1, b1], [a2, b2]...

zip([of('a', 'b', 'c'), of(1, 2, 3)]).subscribe(([a, b]) => console.log(a, b));`;
...
merge(a$, b$).subscribe(val => console.log(val));`;
...
const b$ = of('second');
concat(a$, b$).subscribe(val => console.log(val));`;
